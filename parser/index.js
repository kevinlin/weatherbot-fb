...
let currentTemperature = (location, response) => {
    if (response.icon) {
        let { icon, temperature, apparentTemperature } = response;
        return `Right now, temperature in ${location.toUpperCase()} is ${getFeel(Number(temperature))} at ${temperature.toString()} degrees, feels like ${apparentTemperature.toString()} degrees.`;
    } else {
